...
while not authenticated:
    ...
    if username.lower() == "quit":#A FEATURE TO EXIT THE CODE WHENEVER YOU WANT
    	sys.exit(0)
    password = input("Enter your password: ")
    if password.lower() == "quit":#A FEATURE TO EXIT THE CODE WHENEVER YOU WANT
    	sys.exit(0)
...
def receive_messages(sock):
    while True:
        ...
        if sender_username:
            print(f"\nReceived {sender_username}: {data.decode()}")
        else:
...
# Main loop for sending messages
while True:
    LIST = input("LIST AVAILABLE CLIENTS: Y/QUIT \n")
    
    if LIST.lower() == "quit": #A FEATURE TO EXIT THE CODE WHENEVER YOU WANT
    	sys.exit(0)
    if LIST.lower() == "y":
    ...
    if recipient_user.lower() == "quit": #A FEATURE TO EXIT THE CODE WHENEVER YOU WANT
    	sys.exit(0)
    message = input("Enter message to send: ")
    if message.lower() == "quit":#A FEATURE TO EXIT THE CODE WHENEVER YOU WANT
    	sys.exit(0)
